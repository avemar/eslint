 ...
 * @property {number} diffErrorCount Number of errors on git diff lines.
 * @property {number} diffWarningCount Number of warnings on git diff lines.
 * @property {number} diffFixableErrorCount Number of fixable errors on git diff lines.
 * @property {number} diffFixableWarningCount Number of fixable warnings on git diff lines.
...
function calculateStatsPerFile(messages) {
    return messages.reduce((stat, message) => {
        if (message.fatal || message.severity === 2) {
            ...
            if (message.isDiff) {
                stat.diffWarningCount++;

                if (message.fix) {
                    stat.diffFixableWarningCount++;
                }
            }
        }
        return stat;
 ...
 * @param {Array} diffLines Show output only for these lines
 * @param {Linter} linter Linter context
 * @returns {LintResult} The results for linting on this text.
 * @private
 */
function processText(text, configHelper, filename, fix, allowInlineConfig, reportUnusedDisableDirectives, diffLines, linter) {
    let filePath,
        fileExtension,
        processor;
    ...
    const result = {
        ...
        diffErrorCount: stats.diffErrorCount,
        diffWarningCount: stats.diffWarningCount,
        diffFixableErrorCount: stats.diffFixableErrorCount,
        diffFixableWarningCount: stats.diffFixableWarningCount
    };
